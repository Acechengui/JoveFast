...
import com.jovefast.common.core.annotation.Excel.Type;
import com.jovefast.common.core.annotation.Excels;
...
public class ExcelUtil<T>
{
    ...
    public void init(List<T> list, String sheetName, String title, Type type,boolean freezepane,boolean autofilter)
    {
        if (list == null)
        {
        ...
        createExcelField();
        createWorkbook();
        createTitle(freezepane,autofilter);
        createSubHead();
    }

    /**
     * 创建excel第一行标题
     * @param freezepane 是否冻结窗格
     * @param autofilter 是否自动过滤
     */
    public void createTitle(boolean freezepane,boolean autofilter)
    {
        if (StringUtils.isNotEmpty(title))
        {
            ...
            if(freezepane){
                //冻结前n列，前1行  this.fields.size()
                sheet.createFreezePane(this.fields.size(), 1, this.fields.size(), 1);
            }
            if(autofilter){
                CellRangeAddress c = new CellRangeAddress(0, 0, 0,
                        this.fields.size() - 1);
    ...
    public List<T> importExcel(String sheetName, InputStream is)
    {
        ...
        try
        {
            list = importExcel(sheetName, is, 0);
        }
        catch (Exception e)
        {
            log.error("导入Excel异常{}", e.getMessage());
     ...
     * @param freezepane 是否冻结首行
     * @param autofilter 是否筛选列
     * @return 结果
     */
    public void exportExcel(HttpServletResponse response, List<T> list, String sheetName,boolean freezepane,boolean autofilter)
    {
        exportExcel(response, list, sheetName, StringUtils.EMPTY,freezepane,autofilter);
    }
     ...
     * @param sheetName 工作表的名称
     * @param title 标题
     * @param freezepane 是否冻结表头
     * @param autofilter 是否开启筛选列
     * @return 结果
     */
    public void exportExcel(HttpServletResponse response, List<T> list, String sheetName, String title,boolean freezepane,boolean autofilter)
    {
        response.setContentType("application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
        response.setCharacterEncoding("utf-8");
        this.init(list, sheetName, title, Type.EXPORT,freezepane,autofilter);
        exportExcel(response,false,false);
    }
    ...
    public void importTemplateExcel(HttpServletResponse response, String sheetName, String title)
    {
        response.setContentType("application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
        response.setCharacterEncoding("utf-8");
        this.init(null, sheetName, title, Type.IMPORT,false,false);
        exportExcel(response,false,false);
    }
     ...
     * @return 结果
     */
    public void exportExcel(HttpServletResponse response,boolean freezepane,boolean autofilter)
    {
        try
        {
            writeSheet(freezepane,autofilter);
     ...
     * @param autofilter 是否开启筛选列
     */
    public void writeSheet(boolean freezepane,boolean autofilter)
    {
        // 取出一共有多少个sheet.
        int sheetNo = Math.max(1, (int) Math.ceil(list.size() * 1.0 / sheetSize));
        for (int index = 0; index < sheetNo; index++)
        {
            createSheet(sheetNo, index, freezepane,autofilter);
    ...
    public Cell createHeadCell(Excel attr, Row row, int column)
    {
        ...
        {
            // 填充默认样式，防止合并单元格样式失效
            sheet.setDefaultColumnStyle(column, styles.get(StringUtils.format("data_{}_{}_{}_{}", attr.align(), attr.color(), attr.backgroundColor(),
                    attr.cellType())));
            if (attr.needMerge())
            {
     ...
     * @param autofilter 是否开启筛选列
     */
    public void createSheet(int sheetNo, int index,boolean freezepane,boolean autofilter)
    {
        // 设置工作表的名称.
        if (sheetNo > 1 && index > 0)
        {
            this.sheet = wb.createSheet();
            this.createTitle(freezepane,autofilter);
