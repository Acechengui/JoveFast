...
import com.jovefast.common.core.enums.UserStatus;
...
public class SysLoginService
{
    ...
    @Autowired
    private RemoteParameterService remoteParameterService;

    /**
     * 登录
     */
    public LoginUser login(String username, String pwd) throws Exception {
        String password = RsaUtils.decryptByPrivateKey(pwd);
        // 用户名或密码为空 错误
        ...
        R<Boolean> info = remoteParameterService.getParameterInfo("sys.login.prohibit");
        if ("true".equals(info.getMsg()) && !user.isAdmin()){
            recordLogService.recordLogininfor(username, Constants.LOGIN_FAIL, "系统维护，限制登录中...");
            throw new ServiceException("系统维护，限制登录中...");
        }
